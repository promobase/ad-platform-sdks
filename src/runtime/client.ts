...
export class ApiClient {
  ...
  private async request<T>(method: string, url: string, body?: Record<string, unknown>): Promise<T> {
    if (this.debug) console.log(`[Meta SDK] ${method} ${url}`);
    ...
    if (this.debug) console.log(`[Meta SDK] ${response.status}`, responseBody);
    if (!response.ok) throw FacebookApiError.fromResponse(response.status, responseBody);
