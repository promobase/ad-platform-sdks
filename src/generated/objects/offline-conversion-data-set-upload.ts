...
export function offlineConversionDataSetUploadNode(client: ApiClient, id: string) {
  return {
    ...
    progress: <F extends (keyof Record<string, unknown>)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<Record<string, unknown>, F[number]>>(client, `${id}/progress`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
    pullSessions: <F extends (keyof Record<string, unknown>)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<Record<string, unknown>, F[number]>>(client, `${id}/pull_sessions`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
  };
}
