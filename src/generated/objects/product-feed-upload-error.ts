...
export function productFeedUploadErrorNode(client: ApiClient, id: string) {
  return {
    ...
    samples: <F extends (keyof ProductFeedUploadErrorSampleFields)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<ProductFeedUploadErrorSampleFields, F[number]>>(client, `${id}/samples`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
    suggestedRules: <F extends (keyof ProductFeedRuleSuggestionFields)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<ProductFeedRuleSuggestionFields, F[number]>>(client, `${id}/suggested_rules`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
  };
}
