...
export function adStudyNode(client: ApiClient, id: string) {
  return {
    ...
    cells: <F extends (keyof AdStudyCellFields)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<AdStudyCellFields, F[number]>>(client, `${id}/cells`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
    ...
    instances: {
      list: <F extends (keyof PrivateLiftStudyInstanceFields)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
        new Cursor<Pick<PrivateLiftStudyInstanceFields, F[number]>>(client, `${id}/instances`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
    ...
    objectives: <F extends (keyof AdStudyObjectiveFields)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<AdStudyObjectiveFields, F[number]>>(client, `${id}/objectives`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
  };
}
