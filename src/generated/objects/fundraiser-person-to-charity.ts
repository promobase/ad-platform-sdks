...
export function fundraiserPersonToCharityNode(client: ApiClient, id: string) {
  return {
    ...
    donations: <F extends (keyof Record<string, unknown>)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<Record<string, unknown>, F[number]>>(client, `${id}/donations`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
    ...
    externalDonations: {
      list: <F extends (keyof Record<string, unknown>)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
        new Cursor<Pick<Record<string, unknown>, F[number]>>(client, `${id}/external_donations`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
