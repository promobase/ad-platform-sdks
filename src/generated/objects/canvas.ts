...
export function canvasNode(client: ApiClient, id: string) {
  return {
    ...
    preview: <F extends (keyof CanvasPreviewFields)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<CanvasPreviewFields, F[number]>>(client, `${id}/preview`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
    previews: <F extends (keyof TextWithEntitiesFields)[]>(opts: { fields: F; params?: CanvasListPreviewsParams }) =>
      new Cursor<Pick<TextWithEntitiesFields, F[number]>>(client, `${id}/previews`, opts as { fields: readonly string[]; params?: Record<string, unknown> }),
  };
}
