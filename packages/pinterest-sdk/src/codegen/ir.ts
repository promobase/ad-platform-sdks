...
export function pinterestCanonicalIr(
  ...
  for (const endpoint of [...endpoints].sort((a, b) => a.path.localeCompare(b.path))) {
    ...
    const capabilityId = `pinterest.${category}.${effect === "read" ? "read" : "manage"}`;
    ...
    irEndpoints.push({
      id,
      operationId: `pinterest.${category}.${camel(baseName)}${count === 1 ? "" : count}`,
      platform: "pinterest",
      ...
      description: `Generated from the Pinterest REST API v5 OpenAPI spec (${endpoint.operationId})`,
    });
  }
