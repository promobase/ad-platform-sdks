...
export interface GraphRateLimiterOptions {
  ...
  onThrottle?: (info: { waitMs: number; usage: GraphUsage }) => void;
}
...
export class GraphRateLimiter implements RateLimiter {
  private usage: GraphUsage = { callCount: 0, cpuTime: 0, totalTime: 0 };
  private pausedUntil: number = 0;
  private readonly highWaterMark: number;
  private readonly fallbackWaitMs: number;
  private readonly onThrottle?: GraphRateLimiterOptions["onThrottle"];

  constructor(opts?: GraphRateLimiterOptions) {
        ...
        for (const key of Object.keys(parsed)) {
          ...
          if (Array.isArray(entries)) {
            for (const entry of entries) {
              ...
              // If the Graph API gives us an explicit wait time
