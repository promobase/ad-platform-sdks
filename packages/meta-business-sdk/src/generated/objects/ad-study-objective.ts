...
export function adStudyObjectiveNode(client: ApiClient, id: string) {
  return {
    ...
    brandRequests: <F extends (keyof BrandRequestFields)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<BrandRequestFields, F[number]>>(client, `${id}/brand_requests`, opts as { fields: readonly string[]; params?: Record<string, unknown> }, metaPagination()),
    ...
    offlineConversionDataSets: <F extends (keyof OfflineConversionDataSetFields)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<OfflineConversionDataSetFields, F[number]>>(client, `${id}/offline_conversion_data_sets`, opts as { fields: readonly string[]; params?: Record<string, unknown> }, metaPagination()),
    partnerPrivateStudies: <F extends (keyof BusinessFields)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<BusinessFields, F[number]>>(client, `${id}/partner_private_studies`, opts as { fields: readonly string[]; params?: Record<string, unknown> }, metaPagination()),
    partnerstudies: <F extends (keyof PartnerStudyFields)[]>(opts: { fields: F; params?: Record<string, unknown> }) =>
      new Cursor<Pick<PartnerStudyFields, F[number]>>(client, `${id}/partnerstudies`, opts as { fields: readonly string[]; params?: Record<string, unknown> }, metaPagination()),
  };
}
