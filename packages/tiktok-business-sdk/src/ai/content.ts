...
export function createTikTokContentTools(opts: TikTokClientOptions) {
  ...
  return {
    tt_publish_video: tool({
      description:
        "Publish a video to TikTok. Returns a share_id for tracking publish status. Rate limit: 6/min, 15/day.",
      inputSchema: z.object({
        videoUrl: z
          .string()
          .describe("Public HTTPS URL of the video (.mp4/.mov/.webm, max 1GB, 3-600s)"),
        caption: z
          .string()
          .optional()
          .describe("Video caption (max 2200 chars, supports #hashtags and @mentions)"),
        privacyLevel: z
        ...
        customThumbnailUrl: z
          .string()
          .optional()
          .describe("Custom cover image URL (JPG/PNG/WebP, max 20MB)"),
    ...
    tt_publish_photo: tool({
      description:
        "Publish a photo post to TikTok. Supports up to 35 images. Rate limit: 6/min, 15/day.",
      inputSchema: z.object({
        photoUrls: z
          .array(z.string())
          .min(1)
          .max(35)
          .describe("Public HTTPS URLs of photos (JPG/JPEG/WebP only, max 20MB each)"),
        privacyLevel: z
          .enum(["PUBLIC_TO_EVERYONE", "MUTUAL_FOLLOW_FRIENDS", "FOLLOWER_OF_CREATOR", "SELF_ONLY"])
    ...
    tt_get_video_settings: tool({
      description:
        "Get the account's video posting settings (privacy options, max duration, disabled features).",
      inputSchema: z.object({}),
    ...
    tt_get_hashtag_suggestions: tool({
      description:
        "Get recommended hashtags for a keyword. Returns up to 10 suggestions with view counts.",
      inputSchema: z.object({
