...
export function createTikTokAccountTools(opts: TikTokClientOptions) {
  ...
  return {
    tt_get_account: tool({
      description:
        "Get TikTok account profile info and metrics (display name, followers, bio, etc.).",
      inputSchema: z.object({
        fields: z
          .array(z.string())
          .optional()
          .describe("Fields to fetch (default: display_name, profile_image)"),
        startDate: z
          .string()
          .optional()
          .describe("Start date for metrics (YYYY-MM-DD, max 60 days lookback)"),
    ...
    tt_get_benchmarks: tool({
      description:
        "Get industry benchmarks for average likes, comments, shares, engagement rate, followers, and video views.",
      inputSchema: z.object({
        category: z
          .enum([
            "ART_AND_CRAFTS",
    ...
    tt_spark_ads_enable: tool({
      ...
      inputSchema: z.object({
        ...
        authorizationDays: z
          .enum(["7", "30", "60", "180", "365"])
          .optional()
          .describe("Validity period in days (default: 30)"),
      }),
      execute: async ({ itemId, authorizationDays }) =>
        tt.sparkAds.enable(
          itemId,
          authorizationDays ? (Number(authorizationDays) as 7 | 30 | 60 | 180 | 365) : undefined,
        ),
    }),
