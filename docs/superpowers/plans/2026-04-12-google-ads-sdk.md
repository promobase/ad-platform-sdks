...
**Goal:** Scaffold `@mosaic/google-ads` — a type-safe, fetch-based Google Ads API v23 SDK generated from protobuf definitions via REST transcoding.

**Architecture:** Three layers — (1) shared `HttpClient` in `@mosaic/sdk-runtime` for JSON/fetch/retry/rate-limit primitives, (2) codegen pipeline that loads v23 protos via `protobufjs`, extracts `google.api.http` REST bindings, and emits typed services, and (3) a thin runtime wrapper exposing the `Google` namespace. All ergonomic resource wrappers and AI tools are deferred.

**Tech Stack:** TypeScript, Bun, `protobufjs` (codegen only), `@mosaic/sdk-runtime`, Google Ads API v23 REST.
...
- `package.json` — add `@mosaic/google-ads` workspace dep
...
## Task 2: Scaffold `@mosaic/google-ads` package
...
```md
| `@mosaic/google-ads` | `packages/google-ads-sdk/` | Yes |
```

- [ ] **Step 6: Install deps at workspace root**

Run: `bun install`
Expected: adds `protobufjs`, links `@mosaic/google-ads` into the workspace.
...
export function emitService(s: ServiceAst, messageIndex: Map<string, MessageAst>): string {
  ...
  const importBlock =
    usedTypes.size > 0
      ...
      : "";
  return `${importBlock}import type { HttpClient } from "@mosaic/sdk-runtime";\n\n// Generated from ${s.fullName}. Do not edit by hand.\nexport const ${instance} = {\n${methodNames.join(",\n\n")}\n};\n`;
}
...
import { HttpClient, type RateLimiter, type DelayFn } from "@mosaic/sdk-runtime";
...
- [ ] `@mosaic/google-ads` exists and is installed in the workspace.
- [ ] `bun run codegen` produces a committed `src/generated/v23/` tree that typechecks.
- [ ] `Google.createClient`, `Google.Ads.googleAdsService.search`, `Google.Ads.paginate`, and `Google.Errors.GoogleAdsError` are reachable from `@mosaic/google-ads` and `@mosaic/ad-platforms`.
