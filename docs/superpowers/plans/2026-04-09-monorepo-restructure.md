...
git commit -m "feat: add @mosaic/sdk-runtime with generic client, cursor, and errors"
```
...
With:
```ts
out.push(`import type { ApiClient } from "@mosaic/sdk-runtime";`);
```
...
With:
```ts
out.push(`import { Cursor } from "@mosaic/sdk-runtime";`);
...
To:
```ts
factoryLines.push(`import { ApiClient, type ApiClientOptions } from "@mosaic/sdk-runtime";`);
...
```bash
git add -A
git commit -m "refactor: update codegen emitter for @mosaic/sdk-runtime imports"
```
...
All tests that import from `../../src/runtime/` need updating to either `@mosaic/sdk-runtime` or the new local paths. All tests that import from `../../src/codegen/` or `../../src/generated/` stay as relative paths (they're within the same package now).
...
```bash
git add -A
git commit -m "feat: regenerate Meta SDK with @mosaic/sdk-runtime imports"
```
...
|---------|------|-----------|
| `@mosaic/sdk-runtime` | `packages/sdk-runtime/` | No (private) |
| `@mosaic/meta` | `packages/meta-business-sdk/` | Yes |
...
- Bun for everything
- Each SDK depends on `@mosaic/sdk-runtime` via `workspace:*`
